...
VirtualFS.prototype.mkdtemp = function (prefix, callback) {
  ...
  process.nextTick(function () {
    try {
      return callback(null, utils.invoke(this.base.mkdtemp, [prefix], {
        EACCES: function (error) {
          return errors.EACCES({syscall: 'mkdtemp', path: error.path});
        },
        ENOENT: function (error) {
          return errors.ENOENT({syscall: 'mkdtemp', path: error.path});
        },
        ENOTDIR: function (error) {
          return errors.ENOTDIR({syscall: 'mkdtemp', path: error.path});
        }
      }));
    } catch (error) {
...
VirtualFS.prototype.mkdtempSync = function (prefix) {
  ...
  return utils.invoke(this.base.mkdtemp, [prefix], {
    EACCES: function () {
      return errors.EACCES({syscall: 'mkdtemp', path: prefix + 'XXXXXX'});
    },
    ENOENT: function () {
      return errors.ENOENT({syscall: 'mkdtemp', path: prefix + 'XXXXXX'});
    },
    ENOTDIR: function () {
      return errors.ENOTDIR({syscall: 'mkdtemp', path: prefix + 'XXXXXX'});
    }
  });
};
