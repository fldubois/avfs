...
VirtualFS.prototype.writeFileSync = function (filen, data, options) {
  return utils.invoke(this.base.writeFile, [filen, data, options], function (error) {
    if (error.code === 'path:type') {
      throw new TypeError('path must be a string');
    }
    ...
    return errors[error.code]({syscall: 'open', path: filen});
  });
};
