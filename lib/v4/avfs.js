...
VirtualFS.prototype.fdatasyncSync = function (fd) {
  try {
    return LegacyVFS.prototype.fdatasyncSync.call(this, fd);
  } catch (error) {
    if (error.message === 'Bad argument') {
      throw new TypeError('fd must be a file descriptor');
    }
...
VirtualFS.prototype.mkdirSync = function (path, mode) {
  try {
    return LegacyVFS.prototype.mkdirSync.call(this, path, mode);
  } catch (error) {
    if (error.message === 'Bad argument') {
      throw new TypeError('path must be a string');
    }
...
VirtualFS.prototype.readFileSync = function (filename, options) {
  try {
    return LegacyVFS.prototype.readFileSync.call(this, filename, options);
  } catch (error) {
    if (error.message === 'Bad arguments') {
      throw new TypeError('Expected options to be either an object or a string, but got boolean instead');
    }
...
VirtualFS.prototype.renameSync = function (oldPath, newPath) {
  try {
    ...
    if (['ENOENT', 'EINVAL', 'ENOTDIR', 'EACCES'].indexOf(error.code) !== -1) {
...
VirtualFS.prototype.writeFileSync = function (filename, data, options) {
  try {
    return LegacyVFS.prototype.writeFileSync.call(this, filename, data, options);
  } catch (error) {
    if (error.message === 'Bad arguments') {
      throw new TypeError('Expected options to be either an object or a string, but got boolean instead');
    }
