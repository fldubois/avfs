...
function VirtualFS() {
  ...
  Object.defineProperty(this, 'constants', {
    configurable: false,
    enumerable:   true,
    value:        utils.filter(constants, RegExp.prototype.test.bind(/^[^E][A-Z]*_/))
  });
...
VirtualFS.prototype.copyFileSync = function (src, dest, mode) {
  ...
  errors.nullCheck(src);
  errors.nullCheck(dest);
  ...
  return utils.invoke(this.base.copyFile, [src, dest, mode], function (error) {
    ...
    return errors[error.code]({syscall: 'copyfile', path: src, dest: dest});
  });
};
