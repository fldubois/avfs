...
var parseFlags = function (string) {
  switch (string) {
    case 'r':   // fall through
    ...
    default: throw new Error('Unknown file open flag: ' + string);
  }
};
...
VirtualFS.prototype.ftruncateSync = function (fd, length) {
  ...
  if (!this.handles[fd] || !(this.handles[fd].flags & (flags.RW | flags.WO))) {
    var error = new Error('EBADF, bad file descriptor');
...
VirtualFS.prototype.openSync = function (filepath, flagsString, mode) {
  ...
  var fgs      = parseFlags(flagsString);
  ...
  this.handles[fd] = {
    ...
    path:  filepath,
    read:  0,
    write: (fgs & flags.APPEND) ? get(this.files, elements).length : 0
  };
...
VirtualFS.prototype.readSync = function (fd, buffer, offset, length, position) {
  ...
  if (!this.handles[fd] || !(this.handles[fd].flags & (flags.RW | flags.RO))) {
